 ...
 * 		->i) DFF=0, ii) DFF=1
 *
 * When built-in button pressed, send data
 */
...
void SPI2_Init()
{
	...
	SPI2Handle.SPI_Config.SPI_SSM = SPI_SSM_DI; //Hardware management
...
void GPIO_Button_Init()
{
	...
	GpioButton.GPIO_PinConfig.GPIO_PinPuPdCtrl = GPIO_PUPDCTRL_NO;
	GpioButton.GPIO_PinConfig.GPIO_PinOutType = GPIO_OP_TYPE_PUSHPULL;
...
int main()
{
	...
	//Make NSS signal HIGH when SSM enabled
	//SPI_SSIConfig(SPI2, ENABLE);

	//Make SSOE signal High
	SPI_SSOEConfig(SPI2, ENABLE);

	while(1){
		while(GPIO_ReadFromInputPin(GPIOC, GPIO_PIN_13)); //In Idle state o/p=1, wait...
